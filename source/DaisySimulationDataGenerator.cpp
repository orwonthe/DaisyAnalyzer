...
U32 DaisySimulationDataGenerator::GenerateSimulationData(U64 largest_sample_requested, U32 sample_rate,
  ...
  while (mShiftSimulator->GetCurrentSampleNumber() < largest_sample_requested) {
...
void DaisySimulationDataGenerator::advanceHalfPeriod(double multiple) {
  U32 samples = (U32) (multiple + 0.55);
  mDaisySimulationChannels.AdvanceAll(1); //insert 10 bit-periods of idle
//  mDaisySimulationChannels.AdvanceAll(samples); //insert 10 bit-periods of idle
